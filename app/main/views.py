...
from . import main
from ..models import User_pitch,Pitch,Comments,PitchCategory,Votes
from .. import db
...
def new_pitch(id):
    ...
    if form.validate_on_submit():
        ...
        new_pitch= Pitch(content=content,pitchcategory_id= category.id,user_pitch_id=current_user.id)
...
def new_category():
    ...
    title = 'New category'
    return render_template('new_categories.html', category_form = form,title=title)
...
def view_pitch(id):
    ...
    return render_template('view_pitch.html', pitches=pitches, comment=comment, pitchcategory_id=id)
...
def post_comment(id):
    ...
    if form.validate_on_submit():
        ...
        new_comment = Comments(opinion=opinion, user_id=current_user.id, pitch_id=pitches.id)
        new_comment.save_comment()
        return redirect(url_for('.view_pitch', id=pitches.id))

    return render_template('add-comments.html', comment_form=form, title=title)
